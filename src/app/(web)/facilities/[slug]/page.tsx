...
const FacilityDetails = (props: { params: { slug: string } }) => {
  ...
  const { data: facility, error, isLoading } = useSWR('/api/room', fetchFacility);
  ...
  const handleBookNowClick = async () => {
    ...
    if (participants > facility.numberOfSeats)
      return toast.error("Cannot provide enough seats.");
    if (checkinDate > checkoutDate)
      return toast.error('Please choose a valid checkin period');
    const numberOfDays = calcNumDays();
    const hotelRoomSlug = facility.slug.current;
  };
            ...
            <div className='md:col-span-8 md:w-full'>
              ...
              <div className='flex my-11'>
                {facility.offeredAmenities.map(amenity => (
                  ...
                  >
                    <i className={`fa-solid ${amenity.icon} md:text-2xl`}></i> {/*not working*/}
